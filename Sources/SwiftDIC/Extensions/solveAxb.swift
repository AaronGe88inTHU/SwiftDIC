...
func solveAxb(_ lhs: Matrix<Float>, _ rhs: [Float]) -> [Float]{
    ...
    var identifiedFlatten = Matrix<Float>.identity(size: Int(lhs.rows)).reduce([]) { partialResult, next in
        partialResult + next
    }
...
@available(iOS 11, *)
@available(macOS 10.13, *)
func solveAxb(_ lhs: Matrix<Double>, _ rhs: [Double]) -> [Double]{
                ...
                identifiedFlatten.withUnsafeMutableBufferPointer { idtPtr in
                    result.withUnsafeMutableBufferPointer { resPtr in
                        rhsv.withUnsafeMutableBufferPointer { rhsPtr in
                            ...
                            let x = DenseVector_Double(count: Int32(lhs.rows), data: resPtr.baseAddress!)
                            let b = DenseVector_Double(count: Int32(lhs.rows), data: rhsPtr.baseAddress!)
